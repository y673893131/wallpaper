...
QFrameLessWidget::QFrameLessWidget(QWidget *parent)
    : QWidget(parent), m_drag(DragMove_None),m_bkColor("#80C7ED"),m_bkMode(Back_Color)
{
...
void QFrameLessWidget::resizeEvent(QResizeEvent *event)
{
    ...
    if(height() != qApp->desktop()->availableGeometry().height())
        m_normalSize = size();
    resizeBackground(width(), height(), 0, 0, m_bkColor);//set your like
}
...
void QFrameLessWidget::setBackgroundImg(const QImage& image)
{
    m_bkImg = image;
    m_bkMode = Back_Url;
}
